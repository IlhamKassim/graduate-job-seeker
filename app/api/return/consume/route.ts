import { restoreFromToken } from '@/lib/server/return-visit';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await restoreFromToken(token.trim());
    if (!result.ok) {
    ...
    return Response.json(
      { ok: false, error: 'Could not open that shortlist just now. Try the link again shortly.' },
      { status: 500 },
    );
  }
}
